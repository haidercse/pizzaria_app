...
<div class="container mt-4">
    ...
    <form method="GET" class="mb-3">
        ...
        <input type="month" name="month" class="form-control w-25 d-inline-block" value="{{ request('month') }}">
    ...
    @forelse($tasks as $date => $dailyTasks)
        <div class="card mb-3 shadow-sm">
            ...
            <ul class="list-group list-group-flush" id="task-list-{{ $date }}">
                @foreach ($dailyTasks as $task)
                    <li class="list-group-item d-flex justify-content-between align-items-center" id="task-{{ $task->id }}">
                        ...
                        <div>
                            <button class="btn btn-sm btn-success me-2 save-task" data-id="{{ $task->id }}">Save</button>
            ...
            <div class="card-footer">
                <button type="button" class="btn btn-primary btn-sm" onclick="addTask('{{ $date }}')">Add Task</button>
            </div>
        </div>
    @empty
        <div class="alert alert-info">No tasks found for this month.</div>
    @endforelse
</div>
...
function addTask(date) {
    ...
    li.querySelector('.save-task-new').addEventListener('click', function() {
        ...
        fetch("{{ route('tasks.daily.store') }}", {
            ...
            body: JSON.stringify({ name, date, day_time: 'daily' })
        })
        .then(res => res.json())
        .then(data => {
...
function attachTaskEvents(li){
    ...
    li.querySelector('.save-task')?.addEventListener('click', function(){
        ...
        fetch(`{{ url('tasks/daily-tasks') }}/${id}`, {
            ...
            body: JSON.stringify({ name, day_time:'daily' })
        })
        .then(res => res.json())
        .then(data => {
